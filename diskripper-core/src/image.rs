...
impl Imager {
    ...
    /// Stream drive to disk using native Windows handle or PowerShell fallback
    async fn stream_drive_to_disk(&self, tracker: &ProgressTracker) -> Result<(), DiskRipperError> {
        ...
        let batch_size = 200u32; // Read 200 sectors at a time
        ...
        let total_sectors = if self.total_size > 0 {
            ...
            100_000u64 // Conservative default
        };
        ...
        loop {
            ...
            // Check cancellation
            let status = self.job_manager.get_status(&self.job_id);
            if matches!(status, JobStatus::Cancelled | JobStatus::Failed) {
                return Err(DiskRipperError::Io("Job cancelled".to_string()));
            }

            // Try CDDA read first, then standard sector read
            let batch_result = read_raw_cdda(&self.source_path, current_sector, batch_size)
                .or_else(|_| read_raw_sectors(&self.source_path, current_sector, batch_size, 2048));
            ...
            match batch_result {
                ...
                Err(e) => {
                    ...
                    if consecutive_errors >= MAX_CONSECUTIVE_ERRORS {
                        error!(job_id = %self.job_id, "Too many consecutive errors, stopping");
                        break;
                    }
                    current_sector += batch_size as u64;
                }
